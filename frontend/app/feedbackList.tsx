...
const FeedbackList = () => {
  ...
  useEffect(() => {
    const fetchFeedbacks = async () => {
      try {
        const response = await axios.get(`${process.env.EXPO_PUBLIC_API_URL}/feedbacks/`);
        const tmpFeedbacks = response.data.map((item, index) => ({
          ...item,
          id: `${index}` // Acrescenta índice para torná-lo único
        }));
    ...
    <View style={styles.container}>
      ...
      {feedbacks.length === 0 ? (
        ...
        <FlatList
          ...
          keyExtractor={(item) => item.user_name + item.id + item.created_at}
        />
      )}
    </View>
  );
